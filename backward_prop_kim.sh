...
g++ -O3 -std=c++17 -fopenmp backward_prop_kim.cpp -o backward_prop_kim -I/usr/local/include/pbc -L/usr/local/lib -lpbc -lgmp -lcrypto
...
for i in $(seq 1 5); do
	echo "======= Run $i ======="
	time ./backward_prop_kim 2>&1 | tee -a "$run_log"
	echo
done
