...
EncryptedLookupTable BuildEncryptedLookupTable(pairing_t pairing, PublicKey* pk, int min_x, int max_x, int r3, int r2, element_t alpha, element_t beta, element_t det_B, int z1, int z4, element_t betad[OUTPUT_SIZE], element_t Bstar[OUTPUT_SIZE][HIDDEN_SIZE][HIDDEN_SIZE], int idx = 0) {
    EncryptedLookupTable lut;
    lut.min_x = min_x;
    lut.max_x = max_x;
    lut.num_entries = 0;
...
#pragma omp for schedule(static)
        for (int x = min_x; x <= max_x; x++) {
            ...
            for (int feature_idx = 0; feature_idx < OUTPUT_SIZE; feature_idx++) {
                for (int batch_idx = 0; batch_idx < HIDDEN_SIZE; batch_idx++) {
...
bool MapLTinGTtoG1WithEncryptedLUT(pairing_t pairing,
    ...
    for (size_t idx : candidates) {
        ...
        size_t expected_len = static_cast<size_t>(OUTPUT_SIZE) *
...
int invert_and_det_matrix_Fq_batch(pairing_t pairing,
    ...
    for (int i = 0; i < HIDDEN_SIZE; i++) {
        for (int j = 0; j < HIDDEN_SIZE; j++) {
            ...
            element_init_Zr(aug[i][j + (HIDDEN_SIZE)], pairing);
            if (i == j) {
                element_set1(aug[i][j + (HIDDEN_SIZE)]);
            } else {
                element_set0(aug[i][j + (HIDDEN_SIZE)]);
            }
        }
    }
    ...
    for (int i = 0; i < HIDDEN_SIZE; i++) {
        for (int j = 0; j < HIDDEN_SIZE; j++) {
            element_set(inverse[i][j], aug[i][j + (HIDDEN_SIZE)]);
        }
    }
...
#pragma omp parallel for collapse(2) schedule(static)
        ...
        if (!failed) {
            for (int sample = 0; sample < BATCH_SIZE; sample++) {
                ...
                bool eq = true;
                for (int feature_idx = 0; feature_idx < OUTPUT_SIZE && eq; feature_idx++) {
                    for (int batch_idx = 0; batch_idx < HIDDEN_SIZE; batch_idx++) {
                        element_mul(expected_slot_exp,
                                    betad[feature_idx],
                                    Bstar[feature_idx][0][batch_idx]);
