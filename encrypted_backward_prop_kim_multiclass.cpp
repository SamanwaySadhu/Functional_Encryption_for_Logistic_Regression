...
// --- Multiclass (3-way) extension of encrypted_backward_prop_kim.cpp ---
//
// Unlike the forward pass, the encrypted backward pass needs NO cross-class
// combination: each class c has its own weight vector w_c, its own
// (yhat_c - y_c) prediction error per sample, and therefore its own,
// completely independent gradient/update computation per feature. There is no
// joint LUT here (nothing analogous to the forward pass's softmax LUT) -- the
// entire single-class pipeline (Setup/KeyGen/Encrypt, the GT->GT LUT, the
// GT->G1 LUT, and every Decrypt phase) is simply run NUM_CLASSES times, once
// per class, in exactly the same shape as the single-class file. The only
// things that must be consistent *within* a class (but independent *across*
// classes) are the OTP scale r2/Z1 used to re-mask the weight update for the
...
#define TOTAL_UNITS (NUM_CLASSES * FEATURE_SIZE)
...
EncryptedLookupTable BuildEncryptedLookupTable(pairing_t pairing, PublicKey* pk, int min_x, int max_x, int z3mz4, int z1, element_t alpha, element_t beta, element_t det_B) {
    ...
    std::vector<long> hkdf_fail_by_thread(static_cast<size_t>(thread_count), 0);
    std::vector<long> rand_fail_by_thread(static_cast<size_t>(thread_count), 0);
    std::vector<long> aes_fail_by_thread(static_cast<size_t>(thread_count), 0);
...
#pragma omp parallel
    {
        ...
        long hkdf_fail = 0, rand_fail = 0, aes_fail = 0;
...
#pragma omp for schedule(static)
        ...
        hkdf_fail_by_thread[static_cast<size_t>(tid)] = hkdf_fail;
        rand_fail_by_thread[static_cast<size_t>(tid)] = rand_fail;
        aes_fail_by_thread[static_cast<size_t>(tid)] = aes_fail;
    ...
    long total_hkdf_fail = 0, total_rand_fail = 0, total_aes_fail = 0;
    for (int t = 0; t < thread_count; t++) {
        total_hkdf_fail += hkdf_fail_by_thread[static_cast<size_t>(t)];
        total_rand_fail += rand_fail_by_thread[static_cast<size_t>(t)];
        total_aes_fail += aes_fail_by_thread[static_cast<size_t>(t)];
    }
    if (total_hkdf_fail || total_rand_fail || total_aes_fail) {
        printf("[LUT DIAGNOSTIC] GT->GT build: %zu candidates, %zu produced, hkdf_fail=%ld rand_fail=%ld aes_fail=%ld\n",
               candidate_count, entries.size(), total_hkdf_fail, total_rand_fail, total_aes_fail);
    }
...
// --- 4. Decrypt Algorithm (Kim et al. Section 3) [reference/unused: main()
// inlines every phase directly for fine-grained timing, exactly as in the
// single-class file. Kept for parity/documentation purposes.] ---
...
// One independent instance per (class, feature) unit; flattened into a single
// array of TOTAL_UNITS = NUM_CLASSES * FEATURE_SIZE entries, indexed as
// states[class_idx * FEATURE_SIZE + feature_idx].
...
// Deduplicated failure/final cleanup path (the single-class file repeats this
// block, unchanged, at every one of its six failure points plus once more at
// the very end -- factored out here since the multiclass version needs it in
// exactly as many places). Guarded per-unit by the same flags the single-class
// file already used, so it is always safe to call regardless of how far a
// given unit's pipeline actually got.
void ClearAllUnitsAndSharedState(FeaturePhaseState* states, int total_units,
                                  element_t betad[NUM_CLASSES][BATCH_SIZE],
                                  element_t Bstar[NUM_CLASSES][BATCH_SIZE][FEATURE_SIZE + 1][FEATURE_SIZE + 1]) {
    for (int u = 0; u < total_units; u++) {
        FeaturePhaseState& s = states[u];
        if (s.phase1_init) {
            element_clear(s.D1);
            element_clear(s.D2);
    ...
    for (int c = 0; c < NUM_CLASSES; c++) {
        for (int batch_idx = 0; batch_idx < BATCH_SIZE; batch_idx++) {
            element_clear(betad[c][batch_idx]);
            for (int i = 0; i < FEATURE_SIZE + 1; i++) {
                for (int j = 0; j < FEATURE_SIZE + 1; j++) {
                    element_clear(Bstar[c][batch_idx][i][j]);
                }
            }
        }
    }
...
int main() {
    ...
    // Per-class OTP scale for the next-stage G1/GT embeddings (r2/Z1). These
    // must be shared across all FEATURE_SIZE units of the SAME class, since
    // that class's per-feature outputs get combined (via Bstar's secret
    // sharing) into one next-stage ciphertext per class -- but independent
    // across classes, since each class's combination is entirely separate.
    // r1/Z3 are already per-feature-additive offsets and are simply given
    // their own per-class copy.
    int r2[NUM_CLASSES], Z1[NUM_CLASSES];
    int r1[NUM_CLASSES][FEATURE_SIZE], Z3[NUM_CLASSES][FEATURE_SIZE];
    for (int c = 0; c < NUM_CLASSES; c++) {
        r2[c] = generate_random_int(-(1 << 15), (1 << 15) - 1);
        Z1[c] = generate_random_int(-(1 << 15), (1 << 15) - 1);
        for (int i = 0; i < FEATURE_SIZE; i++) {
            r1[c][i] = generate_random_int(-(1 << 15), (1 << 15) - 1);
            Z3[c][i] = generate_random_int(-(1 << 15), (1 << 15) - 1);
        }
    }

    // Per-class batch-combination secret sharing material (mirrors the
    // single-class scheme's betad/Bstar, just replicated once per class).
    element_t betad[NUM_CLASSES][BATCH_SIZE];
    element_t Bstar[NUM_CLASSES][BATCH_SIZE][FEATURE_SIZE + 1][FEATURE_SIZE + 1];
    for (int c = 0; c < NUM_CLASSES; c++) {
        for (int batch_idx = 0; batch_idx < BATCH_SIZE; batch_idx++) {
            element_init_Zr(betad[c][batch_idx], pairing);
            element_random(betad[c][batch_idx]);
            ...
            for (int i = 0; i < FEATURE_SIZE + 1; i++) {
                for (int j = 0; j < FEATURE_SIZE + 1; j++) {
                    ...
                    element_init_Zr(B_inv[i][j], pairing);
                    element_init_Zr(Bstar[c][batch_idx][i][j], pairing);
                }
            }
            ...
            for (int i = 0; i < FEATURE_SIZE + 1; i++) {
                for (int j = 0; j < FEATURE_SIZE + 1; j++) {
                    ...
                    element_set(Bstar[c][batch_idx][i][j], tmp);
                }
            }
    ...
    // The GT->GT LUT's domain is the range of output_values = sum_i
    // (yhat_i - y_i) * x_i, where (yhat_i - y_i) in [-MAX_X, MAX_X] and x_i in
    // [MIN_X, MAX_X]. Since |MIN_X| = MAX_X + 1 > MAX_X, the extremal product
    // is reached via MIN_X, not MAX_X, so the safe bound uses -MIN_X wherever
    // MAX_X alone would under-cover the true range (same asymmetry as in the
    // forward-pass multiclass file). The GT->G1 LUT's domain (the combined
    ...
    const int ebp_gtg1_min_x = -ebp_gtg1_bound;
    const int ebp_gtg1_max_x = ebp_gtg1_bound;

    FeaturePhaseState states[TOTAL_UNITS] = {};
    for (int u = 0; u < TOTAL_UNITS; u++) {
        int c = u / FEATURE_SIZE;
        int feature = u % FEATURE_SIZE;
        states[u].class_idx = c;
        states[u].feature_idx = feature;
        states[u].r1_feature = r1[c][feature];
        states[u].Z3_feature = Z3[c][feature];
        for (int i = 0; i < BATCH_SIZE; i++) {
            for (int j = 0; j < FEATURE_SIZE + 1; j++) {
                states[u].gt_wd_over_l_in_g1_init[i][j] = false;
            }
        }
    }
    ...
    // Phase 0: setup + keygen + encrypt + LUT builds for every (class, feature) unit.
    for (int u = 0; u < TOTAL_UNITS; u++) {
        int c = u / FEATURE_SIZE;
        int feature = u % FEATURE_SIZE;
        ...
        if (expected_output != (z1 * output_values + z3 - z4)) {
            printf("\n[ASSERTION FAILED] expected_output != z1 * output_values + z3 - z4 at class %d, feature %d\n", c, feature);
            for (int i = 0; i < BATCH_SIZE; i++) {
            ...
            ClearAllUnitsAndSharedState(states, TOTAL_UNITS, betad, Bstar);
        ...
        start = std::chrono::steady_clock::now();
        s.lut = BuildEncryptedLookupTable(pairing, &s.pk, ebp_gtgt_min_x, ebp_gtgt_max_x, z3 - z4, z1, s.alpha_sample, s.beta_sample, s.msk.det_B);
        stop = std::chrono::steady_clock::now();
        ...
        start = std::chrono::steady_clock::now();
        s.lut_gt_to_g1 = BuildEncryptedLookupTableGTtoG1(pairing, &s.pk, ebp_gtg1_min_x, ebp_gtg1_max_x, z1, z4, r2[c], s.r1_feature, Z1[c], s.Z3_feature, betad[c], Bstar[c], feature);
        stop = std::chrono::steady_clock::now();
        total_lut_build_ms += std::chrono::duration<double, std::milli>(stop - start).count();
        total_lut_size_bytes += static_cast<long double>(estimate_lut_size_bytes(s.lut_gt_to_g1));

        if (u == 0) {
            printf("\n=== LUT Row Stats class 0, feature 0 ===\n");
        ...
        printf("class %d feature %d phase-0 time: %.3f s\n", c, feature, sample_total_ms / 1000.0);
    }
    ...
    for (int u = 0; u < TOTAL_UNITS; u++) {
        ...
        if (element_cmp(D1_expected, s.D2) != 0) {
            printf("[ASSERTION FAILED] D1^expected_output != D2 at class %d, feature %d\n", s.class_idx, s.feature_idx);
            element_clear(expected_exp);
            element_clear(D1_expected);
            ClearAllUnitsAndSharedState(states, TOTAL_UNITS, betad, Bstar);
            pairing_clear(pairing);
    ...
    if (phase4_failed) {
        printf("Failed to map D2 from GT to GT using encrypted LUT at class %d, feature %d.\n",
               states[phase4_failed_unit].class_idx, states[phase4_failed_unit].feature_idx);
        ClearAllUnitsAndSharedState(states, TOTAL_UNITS, betad, Bstar);
        pairing_clear(pairing);
        return 1;
    }
    ...
    for (int u = 0; u < TOTAL_UNITS; u++) {
        ...
        if (element_cmp(s.L_in_GT, expected_L_in_GT) != 0) {
            printf("[ASSERTION FAILED] L_in_GT mismatch at class %d, feature %d\n", s.class_idx, s.feature_idx);
            element_clear(lut_exp);
            element_clear(expected_L_in_GT);
            ClearAllUnitsAndSharedState(states, TOTAL_UNITS, betad, Bstar);
            pairing_clear(pairing);
    ...
    if (phase7_failed) {
        printf("Failed to map gt_wd_over_l from GT to G1 using encrypted LUT at class %d, feature %d.\n",
               states[phase7_failed_unit].class_idx, states[phase7_failed_unit].feature_idx);
        ClearAllUnitsAndSharedState(states, TOTAL_UNITS, betad, Bstar);
        pairing_clear(pairing);
        return 1;
    }
    ...
    for (int u = 0; u < TOTAL_UNITS; u++) {
        FeaturePhaseState& s = states[u];
        int c = s.class_idx;
        int feature = s.feature_idx;
        long expected_g1_exp_si = static_cast<long>(
            r2[c] * (int)((s.w_scaled - (s.output_values >> (QUANTIZATION_BITS - 1))) * (long double)LR / (long double)BATCH_SIZE) + s.r1_feature);
        ...
        for (int batch_idx = 0; batch_idx < BATCH_SIZE && g1_ok; batch_idx++) {
            for (int feature_idx = 0; feature_idx < FEATURE_SIZE + 1; feature_idx++) {
                element_mul(expected_slot_exp,
                            betad[c][batch_idx],
                            Bstar[c][batch_idx][feature][feature_idx]);
                element_mul(expected_slot_exp, expected_slot_exp, expected_g1_z);
        ...
        if (!g1_ok) {
            printf("[ASSERTION FAILED] gt_wd_over_l_in_g1 mismatch at class %d, feature %d\n", c, feature);
            element_clear(expected_g1_z);
            element_clear(expected_slot_exp);
            element_clear(expected_slot_g1);
            element_clear(expected_gt_exp);
        ...
        long expected_gt_exp_si =
            static_cast<long>(Z1[c] * (s.w_scaled - (s.output_values >> (QUANTIZATION_BITS - 1))) + s.Z3_feature);
        element_set_si(expected_gt_exp, expected_gt_exp_si);
        element_pow_zn(expected_gt, s.pk.gT_base, expected_gt_exp);

        if (element_cmp(s.gt_wd_over_l_in_gt, expected_gt) != 0) {
            printf("[ASSERTION FAILED] gt_wd_over_l_in_gt mismatch at class %d, feature %d\n", c, feature);
            element_clear(expected_g1_z);
            element_clear(expected_slot_exp);
            element_clear(expected_slot_g1);
            element_clear(expected_gt_exp);
    ...
    // Decrypt phase 9 loop (timed): aggregate C2 per class, across that
    // class's FEATURE_SIZE feature-units (no cross-class combination).
    element_t C2[NUM_CLASSES][BATCH_SIZE][FEATURE_SIZE + 1];
    auto phase9_c2_start = std::chrono::steady_clock::now();
    #pragma omp parallel for collapse(3) schedule(static)
    for (int c = 0; c < NUM_CLASSES; c++) {
        for (int batch_idx = 0; batch_idx < BATCH_SIZE; batch_idx++) {
            for (int feature_idx = 0; feature_idx < FEATURE_SIZE + 1; feature_idx++) {
                element_init_G1(C2[c][batch_idx][feature_idx], pairing);
                element_set1(C2[c][batch_idx][feature_idx]);

                for (int feature = 0; feature < FEATURE_SIZE; feature++) {
                    FeaturePhaseState& s = states[c * FEATURE_SIZE + feature];
                    element_mul(C2[c][batch_idx][feature_idx],
                                C2[c][batch_idx][feature_idx],
                                s.gt_wd_over_l_in_g1[batch_idx][feature_idx]);
                }

                element_t tail_exp, tail_term;
                element_init_Zr(tail_exp, pairing);
                element_init_G1(tail_term, pairing);
                element_set(tail_exp, Bstar[c][batch_idx][FEATURE_SIZE][feature_idx]);
                element_pow_zn(tail_term, states[c * FEATURE_SIZE].pk.g1, tail_exp);
                element_mul(C2[c][batch_idx][feature_idx],
                            C2[c][batch_idx][feature_idx],
                            tail_term);
    ...
    for (int c = 0; c < NUM_CLASSES; c++) {
        for (int batch_idx = 0; batch_idx < BATCH_SIZE; batch_idx++) {
            for (int feature_idx = 0; feature_idx < FEATURE_SIZE + 1; feature_idx++) {
                element_clear(C2[c][batch_idx][feature_idx]);
            }
        }
    }
    ...
    printf("\n=== Benchmark Summary (%d classes x %d features) ===\n", NUM_CLASSES, FEATURE_SIZE);
        printf("KeyGen total: %lld us, average: %.3f us\n",
            total_keygen_us,
            static_cast<double>(total_keygen_us) / TOTAL_UNITS);
        printf("Encrypt total: %lld us, average: %.3f us\n",
            total_encrypt_us,
            static_cast<double>(total_encrypt_us) / TOTAL_UNITS);
