...
// --- Multiclass (3-way) extension of backward_prop_kim.cpp ---
//
// Unlike the forward pass, the backward pass needs NO cross-class
...
// per class, in exactly the same shape as the single-class file.
...
EncryptedLookupTable BuildEncryptedLookupTableGTtoG1(pairing_t pairing, PublicKey* pk, int min_x, int max_x, int z1, int z4) {
    EncryptedLookupTable lut;
    lut.min_x = min_x;
    lut.max_x = max_x;
    lut.num_entries = 0;
...
#pragma omp for schedule(static)
        for (int x = min_x; x <= max_x; x++) {
            ...
            int w_i = static_cast<int>((long double)x * (long double)LR / (long double)BATCH_SIZE);
            unsigned char w = static_cast<unsigned char>(w_i & 0xFF);
            element_pow_zn(gt_val, pk->gT_base, expt);

            std::vector<unsigned char> gt_bytes = serialize_element_to_bytes(gt_val);

            std::vector<unsigned char> plaintext(1);
            plaintext[0] = w;
...
bool Decrypt(pairing_t pairing, PublicKey* pk, DecryptionKey* sk, Ciphertext* ct,
    ...
    unsigned char gt_wd_over_l_w = 0;
    // Start of Phase 7
    if (!MapLTinGTtoG1WithEncryptedLUT(pairing, gt_wd_over_l, lut_gt_to_g1, gt_wd_over_l_w, verbose)) {
    ...
    // Start of Phase 8
    unsigned char expected_w = static_cast<unsigned char>(
        static_cast<int>((w - (output_value >> (QUANTIZATION_BITS - 1))) *
                         (long double)LR / (long double)BATCH_SIZE) & 0xFF);
    if (gt_wd_over_l_w != expected_w) {
        printf("[ASSERTION FAILED] gt_wd_over_l_w != expected_w\n");
        element_clear(wd_exp);
        element_clear(gt_wd);
        element_clear(l_in_gt_inv);
        element_clear(gt_wd_over_l);
...
// block, unchanged, at every one of its failure points plus once more at the
// very end -- factored out here since the multiclass version needs it in
// exactly as many places). Guarded per-unit by the same flags the
// single-class file already used, so it is always safe to call regardless of
// how far a given unit's pipeline actually got.
...
int main() {
    ...
    // Per-class copies of the (unused-by-the-arithmetic, carried over from the
    // single-class file's template) r2/Z1/r1/Z3/betad/Bstar scaffolding, so
    // that each class's pipeline is an exact, independent replica of the
    // single-class one.
    ...
    for (int u = 0; u < TOTAL_UNITS; u++) {
        ...
        start = std::chrono::steady_clock::now();
        s.lut_gt_to_g1 = BuildEncryptedLookupTableGTtoG1(pairing, &s.pk, ((BATCH_SIZE) / (LR) + (BATCH_SIZE)) * (MIN_X), ((BATCH_SIZE) / (LR) + (BATCH_SIZE)) * (MAX_X), z1, z4);
        stop = std::chrono::steady_clock::now();
        total_lut_build_ms += std::chrono::duration<double, std::milli>(stop - start).count();
    ...
    for (int u = 0; u < TOTAL_UNITS; u++) {
        FeaturePhaseState& s = states[u];
        if (!s.phase7_w_init) {
            printf("[ASSERTION FAILED] phase7_w_init is false at class %d, feature %d\n", s.class_idx, s.feature_idx);
            ClearAllUnitsAndSharedState(states, TOTAL_UNITS, betad, Bstar);
            pairing_clear(pairing);
            return 1;
        }
        ...
        unsigned char expected_w = static_cast<unsigned char>(
            static_cast<int>((s.w_scaled - (s.output_values >> (QUANTIZATION_BITS - 1))) *
                             (long double)LR / (long double)BATCH_SIZE) & 0xFF);

        if (s.gt_wd_over_l_w != expected_w) {
            printf("[ASSERTION FAILED] gt_wd_over_l_w mismatch at class %d, feature %d\n", s.class_idx, s.feature_idx);
            ClearAllUnitsAndSharedState(states, TOTAL_UNITS, betad, Bstar);
            pairing_clear(pairing);
            return 1;
        }
    ...
    // Decrypt phase 9 loop (cleanup per unit).
    ClearAllUnitsAndSharedState(states, TOTAL_UNITS, betad, Bstar);

    total_decrypt_ms =
        static_cast<long long>(phase2_ms + phase4_ms + phase6_ms + phase7_ms);
