...
int main() {
    ...
    for (int feature = 0; feature < FEATURE_SIZE; feature++) {
        ...
        states[feature].phase7_w_init = false;
    }
    ...
    for (int feature = 0; feature < FEATURE_SIZE; feature++) {
        FeaturePhaseState& s = states[feature];
        if (!s.phase7_w_init) {
            printf("[ASSERTION FAILED] phase7_w_init is false at feature %d\n", feature);
            for (int f = 0; f < FEATURE_SIZE; f++) {
                if (states[f].phase1_init) {
                    element_clear(states[f].D1);
                    element_clear(states[f].D2);
        ...
        if (s.gt_wd_over_l_w != expected_w) {
            printf("[ASSERTION FAILED] gt_wd_over_l_w mismatch at feature %d\n", feature);
            for (int f = 0; f < FEATURE_SIZE; f++) {
                if (states[f].phase1_init) {
                    element_clear(states[f].D1);
                    element_clear(states[f].D2);
    ...
    // Decrypt phase 9 loop (cleanup per feature).
    for (int feature = 0; feature < FEATURE_SIZE; feature++) {
        FeaturePhaseState& s = states[feature];
        if (s.phase6_init) {
