...
g++ -O3 -std=c++17 -fopenmp backward_prop_kim_multiclass.cpp -o backward_prop_kim_multiclass -I/usr/local/include/pbc -L/usr/local/lib -lpbc -lgmp -lcrypto
...
for i in $(seq 1 5); do
	echo "======= Run $i ======="
	time ./backward_prop_kim_multiclass 2>&1 | tee -a "$run_log"
	echo
done
